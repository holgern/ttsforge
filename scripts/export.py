...
"""
From https://raw.githubusercontent.com/hexgrad/kokoro/refs/heads/main/examples/export.py
mkdir checkpoints
...
uv run scripts/export.py --config_file checkpoints/config.json \
    --checkpoint_path checkpoints/kokoro-v1_0.pth
onnxslim onnx/kokoro.onnx onnx/kokoro-v1.0.onnx
"""
...
def export_onnx(model, output):
    ...
    style = torch.randn(1, 256, dtype=torch.float32)

    # Use dynamo=False for legacy export mode (required for PyTorch 2.6+)
    if True:
        speed = torch.rand(1)
        torch.onnx.export(
            model,
...
def load_sample(model):
    ...
    text = """
    In today's fast-paced tech world, building software applications
    has never been easier — thanks to AI-powered coding assistants.'
    """
    text = """
